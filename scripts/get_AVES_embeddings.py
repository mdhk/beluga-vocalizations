...
import pandas as pd
import torch
from beluga_vocalizations.data_loading import get_dataloader, load_recordings_df
from beluga_vocalizations.model_loading import AvesEmbedding
from beluga_vocalizations.paths import AVES_BIO_CONFIG, AVES_BIO_MODEL, ROOT

rec_df = load_recordings_df(ROOT / 'results/recordings.csv').reset_index()

embedding_model = AvesEmbedding(config_path=AVES_BIO_CONFIG, model_path=AVES_BIO_MODEL)
...
emb_df = pd.DataFrame(np.vstack(embeddings), columns=[f'emb{d+1}' for d in range(embedding_model.emb_dim)])
